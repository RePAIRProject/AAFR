...
orig_folder = '/home/lucap/code/AAFR/segmentation_results_synth/synth_03_16_fractured_62'
#exp_name = 'OTHER_DS_03_14_tuwien_brick_pair1'
#pcl_name = 'brick_part02.ply'
target_mesh = o3d.io.read_triangle_mesh(os.path.join(orig_folder, 'piece_0.obj'))
orig_pcl = target_mesh.sample_points_uniformly(number_of_points=50000)
target_mesh1 = o3d.io.read_triangle_mesh(os.path.join(orig_folder, 'piece_1.obj'))
orig_pcl1 = target_mesh1.sample_points_uniformly(number_of_points=50000)
#orig_pcl = o3d.io.read_point_cloud(os.path.join(orig_folder, 'orig.ply'))
regions_pcl = o3d.io.read_point_cloud(os.path.join(orig_folder, "col_regions_piece_0.ply"))
regions_pcl1 = o3d.io.read_point_cloud(os.path.join(orig_folder, "col_regions_piece_1.ply"))
borders_pcl = o3d.io.read_point_cloud(os.path.join(orig_folder, "col_borders_piece_0.ply"))
borders_pcl1 = o3d.io.read_point_cloud(os.path.join(orig_folder, "col_borders_piece_1.ply"))

pred1 = o3d.io.read_point_cloud(os.path.join(orig_folder, "output_pcl", "piece_1_predicted.ply"))


# o3d.visualization.draw_geometries([orig_pcl, orig_pcl1])
# pdb.set_trace()
front = [ -0.87601556407627945, -0.48219264309227672, 0.0093266523363027005 ]
lookat = [ 5.9490751312132883e-05, 1.087888671427728e-05, 2.5492569197260728e-07 ]
up = [ 0.0056368779568508991, 0.0091003568514986151, 0.99994270291455956 ]
zoom = 0.6999999
# cookie 
...
# zoom = 2
# manual 
o3d.visualization.draw_geometries([orig_pcl, orig_pcl1], zoom=zoom,
                                  front=front, lookat=lookat, up=up)

orig_pcl.paint_uniform_color([0, 0, 1])
orig_pcl1.paint_uniform_color([0.9, 0.9, 0])

o3d.visualization.draw_geometries([orig_pcl], zoom=zoom,
                                  front=front, lookat=lookat, up=up)
o3d.visualization.draw_geometries([orig_pcl1], zoom=zoom,
                                  front=front, lookat=lookat, up=up)


o3d.visualization.draw_geometries([borders_pcl], zoom=zoom,
                                  front=front, lookat=lookat, up=up)
o3d.visualization.draw_geometries([borders_pcl1], zoom=zoom,
                                  front=front, lookat=lookat, up=up)


o3d.visualization.draw_geometries([regions_pcl], zoom=zoom,
                                  front=front, lookat=lookat, up=up)
o3d.visualization.draw_geometries([regions_pcl1], zoom=zoom,
                                  front=front, lookat=lookat, up=up)


pred1.paint_uniform_color([0.9, 0.9, 0])
o3d.visualization.draw_geometries([orig_pcl, pred1], zoom=zoom,
                                  front=front, lookat=lookat, up=up)
