...
class FeatureLines(object):
    ...
    def cal_max_angle(self):
        all_angles = []
        for i in tqdm(range(len(self.points_q_points))):
    ...
    def array_to_graph(self):
        Graph = nx.Graph()
        edges = []
        for p in tqdm(range(len(self.pcd.points))):
    ...
    def __create_graph(self,pattern="crease",T=0.9,pattern_length_T=2):
            ...
            while len(edges)>0:
                ...
                if ds.exists(edge[0]) and ds.exists(edge[1]):
                    ...
                    elif self.path_length(tmp_Graph,edge[0],edge[1],pattern_length_T)>pattern_length_T//2:
    ...
    #prunning the graph
    def __remove_point(self,G,p,visited):
        counter = 0
        short_branches = []
        stack = [[p]]
        while stack:
    ...
    def __check_point(self,G,p,visited,threshold):
        counter = 0
        short_branches = []
        stack = [[p]]
        while stack:
    ...
    def create_crease(self,T=0.9,pattern_length_T=2):
            ...
            for point,my_nodes in tqdm(nodes.items()):
                ...
                for node in my_nodes:
                    if not self.__check_point(self.tmp_graph,node,{point},T):
                        score+=1
                    else:
                ...
                if score >= 2:
                    for node in tmp_rem_nodes:
                        tmp_rem_nodes = self.__remove_point(self.tmp_graph,node,{point})
    ...
    def create_border(self,T=0.9,pattern_length_T=2):
        ...
        self.border_pattern_pruned = [[node for group in self.border_pattern] for node in group-self.border_pruned_points]
        print(len(self.border_pruned_points)/len(list(self.pcd.points)))
    def show_heat(self,weights,thre = 0):
        if len(weights.shape)>1weights.shape[-1] > 1:
