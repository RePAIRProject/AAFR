...
def segment_regions(obj, borders_indices, isolated_islands_pruned_graph):   
    ...
    obj.inner_pcd = copy(obj.pcd)
    obj.inner_pcd = obj.inner_pcd.select_by_index(borders_indices, invert=True)

    clusterer = hdbscan.HDBSCAN(min_cluster_size=5)
    labels = clusterer.fit_predict(np.asarray(obj.inner_pcd.points))
    
    regions_col = np.zeros((len(obj.inner_pcd.points), 3))
    seg_parts = []
    for k in range(len(labels)):
        regions_col[k] = colors[labels[k] % len(colors)]

    for k in range(max(labels)+1):
        mask = labels==k
        Obj_tmp = copy(obj)
        Obj_tmp.pcd = copy(obj.inner_pcd)
        Obj_tmp.pcd.points = o3d.utility.Vector3dVector(np.asarray(obj.inner_pcd.points)[mask])
        Obj_tmp.pcd.paint_uniform_color(np.asarray(colors[k % len(colors)]).astype("float") / 255.0)
        seg_parts.append(Obj_tmp)

    obj.inner_pcd.colors = o3d.utility.Vector3dVector(np.asarray(regions_col).astype("float") / 255.0)         

    return seg_parts, labels, obj.inner_pcd
